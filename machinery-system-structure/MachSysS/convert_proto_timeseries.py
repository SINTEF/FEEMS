        ...
        list(
            map(
                lambda each_instance: list(
                    map(lambda each_power: each_power, each_instance.propulsion_power_kw)
                ),
                proto_timeseries.propulsion_power_timeseries,
            )
        )
    )
    assert propulsion_power.shape[0] == len(time), "Time and propulsion power length mismatch"
    assert propulsion_power.shape[1] == len(propulsor_names), (
        "Propulsion power and propulsor names length mismatch"
    )
    data = {
        propulsor_name: propulsion_power[:, i] for i, propulsor_name in enumerate(propulsor_names)
    }
    auxiliary_power = list(
        map(
            lambda each: each.auxiliary_power_kw,
