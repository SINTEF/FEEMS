...
def convert_proto_electric_component_to_feems(
    ...
    return ElectricComponent(
        ...
        eff_curve=convert_proto_efficiency_bsfc_power_to_np_array(proto_component.efficiency),
...
def convert_proto_fuel_cell_system_to_feems(
    ...
    fuel_cell = FuelCell(
        ...
        eff_curve=convert_proto_efficiency_bsfc_power_to_np_array(subsystem.fuel_cell.efficiency),
        fuel_type=TypeFuel(subsystem.fuel_cell.fuel.fuel_type),
        fuel_origin=FuelOrigin(subsystem.fuel_cell.fuel.fuel_origin),
        uid=(subsystem.fuel_cell.uid if len(subsystem.fuel_cell.uid) > _MIN_LENGTH_UID else None),
    )
...
def get_nox_calculation_method(
    ...
    if proto_comp.nox_calculation_method is not None:
        nox_calculation_method = convert_nox_calculation_method(proto_comp.nox_calculation_method)
...
def convert_proto_engine_to_feems(
    ...
    if proto_engine.HasField("pilot_bsfc"):
        return EngineDualFuel(
            ...
            bspfc_curve=convert_proto_efficiency_bsfc_power_to_np_array(proto_engine.pilot_bsfc),
...
def convert_proto_multifuel_engine_to_feems(
    ...
    for fuel_mode in proto_engine.fuel_modes:
        ...
        if (
            ...
            fuel_characteristics.pilot_fuel_type = TypeFuel(fuel_mode.pilot_fuel.fuel_type)
            fuel_characteristics.pilot_fuel_origin = FuelOrigin(fuel_mode.pilot_fuel.fuel_origin)
            fuel_characteristics.bspfc_curve = convert_proto_efficiency_bsfc_power_to_np_array(
                fuel_mode.pilot_bsfc
            )
...
def convert_proto_genset_to_feems(subsystem: proto.Subsystem, switchboard_id: int) -> Genset:
    ...
    if subsystem.HasField("engine"):
        ...
        raise ValueError("The genset engine is not properly defined. Engine field is missing.")
...
def convert_proto_coges_to_feems(subsystem: proto.Subsystem, switchboard_id: int) -> COGES:
    """Converts protobuf subsystem message to feems component"""
...
def convert_proto_switchboard_to_feems(switchboard: proto.Switchboard) -> Switchboard:
    ...
    for subsystem in switchboard.subsystems:
        if subsystem.component_type == proto.Subsystem.ComponentType.FUEL_CELL_SYSTEM:
            ...
            components.append(
                convert_proto_coges_to_feems(subsystem=subsystem, switchboard_id=switchboard_id)
            )
        ...
        if sub_system.component_type == proto.Subsystem.ComponentType.MAIN_ENGINE:
            ...
            components.append(
                MainEngineForMechanicalPropulsion(
                    ...
                    engine=engine,
                    shaft_line_id=shaft_line_id,
                    uid=(sub_system.uid if len(sub_system.uid) > _MIN_LENGTH_UID else None),
                )
            )
            ...
            raise ValueError(
                f"The component type {TypeComponent(sub_system.component_type)} is not supported."
            )
    return ShaftLine(
