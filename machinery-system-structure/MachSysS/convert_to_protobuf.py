...
from feems.types_for_feems import EmissionCurve, NOxCalculationMethod, TypeComponent
...
def convert_np_array_to_protobuf_power_curve(power_curve: np.array) -> proto.PowerCurve:
    ...
    if power_curve.shape[1] == 2:
        curve = proto.Curve1D()
        curve.points.extend(
            [proto.Point(x=each_point[0], y=each_point[1]) for each_point in power_curve]
        )
    ...
    for fuel_mode in engine_feems.multi_fuel_characteristics:
        fuel_modes.append(
            proto.MultiFuelEngine.FuelMode(
                ...
                emission_curves=convert_emission_curves_to_protobuf(fuel_mode.emission_curves),
                nox_calculation_method=convert_nox_calculation_method_to_protobuf(
...
def convert_switchboard_to_protobuf(
    ...
    for component in switchboard_feems.components:
        ...
        if component.type == TypeComponent.GENERATOR:
            ...
            subsystem.battery.CopyFrom(convert_battery_component_to_protobuf(component=component))
...
def convert_shaftline_to_protobuf(shaftline_feems: ShaftLine) -> proto.ShaftLine:
    ...
    for component in shaftline_feems.components:
        ...
        if component.type == TypeComponent.MAIN_ENGINE:
            ...
            if type(component.engine) is Engine or type(component.engine) is EngineDualFuel:
                subsystem.engine.CopyFrom(
                    convert_engine_component_to_protobuf(
