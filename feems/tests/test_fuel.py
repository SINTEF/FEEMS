...
def test_fuel_class():
    ...
    assert len(fuel_by_eu.ghg_emission_factor_tank_to_wake) == 7
    assert fuel_by_eu.get_ghg_emission_factor_tank_to_wake_gco2eq_per_gfuel(
        fuel_consumer_class=FuelConsumerClassFuelEUMaritime.BOILER,
    ) == pytest.approx(2.75 + 0.00011 * _GWP100_N2O)
    assert fuel_by_eu.get_ghg_emission_factor_tank_to_wake_gco2eq_per_gfuel(
